...
update_mv_zpro() {
  print_banner
  printf "${WHITE} 💻 Agora, vamos mover a update até o deployplw...${GRAY_LIGHT}"
  printf "\n\n"

  sleep 2

  sudo su - root <<EOF
  cp "${PROJECT_ROOT}"/update.zip /home/deployplw/
EOF
...
update_delete_frontend() {
  ...
  sudo su - root <<EOF
  cd /home/deployplw/zpro.io/frontend || exit
  rm -rf src
...
update_backend_node_dependencies() {
  ...
  sudo su - deployplw <<EOF
  cd /home/deployplw/zpro.io/backend
  npm install --force
EOF
...
update_frontend_node_dependencies() {
  ...
  sudo su - deployplw <<EOF
  cd /home/deployplw/zpro.io/frontend
  npm install --force
EOF
...
update_start_pm2() {
  print_banner
  printf "${WHITE} 💻 Agora, vamos reiniciar os serviços no deployplw...${GRAY_LIGHT}"
  printf "\n\n"
...
update_success() {
  ...
  printf "\n"
  printf "${GREEN} FAQ: https://plwdesign.online/${NC}"
  printf "\n"
  printf "${GREEN} Suporte: https://plwdesign.online/${NC}"
  printf "\n"
  printf "${CYAN_LIGHT}";
  printf "${NC}";
